...
class SendbirdSdk {
  ...
  factory SendbirdSdk({
    ...
    if (appId == null || (appId == _instance._int.state.appId && apiToken == null && options == null)) {
      return _instance;
    }
  ...
  Future<PushTokenRegistrationStatus> registerPushToken({
    ...
    try {
      ...
      int? deviceTokenLastDeletedAt = await _int.api.send<int?>(UserPushTokenRegisterRequest(
        type: type,
        token: token,
      ...
      logger.e(StackTrace.current, error: "Failed to register push token");
      rethrow;
    }
  }
  ...
  Future<void> unregisterAllPushToken() async {
    ...
    return _int.api.send<int?>(UserPushTokenUnregisterAllRequest()).then((value) {
      //* If deviceTokenLastDeletedAt is not NULL then update the state
      if (value != null) {
        prefs.setInt(
          prefDeviceTokenLastDeletedAt,
  ...
  Future<void> setDoNotDisturb({
    ...
    if (enable && startHour < 0 || startHour > 23 || startMin < 0 || startMin > 59 || endHour < 0 || endHour > 23 || endMin < 0 || endMin > 59) {
      throw InvalidParameterError();
    }
  ...
  Future<void> markAsReadAll() async {
    await _int.api.send(GroupChannelMarkAsReadApiRequest(userId: _int.state.userId));
  }
  ...
  Future<void> markAsDelivered({required Map<String, dynamic> data}) async {
    ...
    if (appId != null && userId != null && channelUrl != null && sessionKey != null && createdAt != null) {
  ...
  /// Returns total number of group channels that contains unread messages
  /// with given [params].
  Future<int> getTotalUnreadChannelCountWithParams(GroupChannelTotalUnreadChannelCountParams params) async {
    return _int.api.send<int>(UserTotalUnreadChannelCountGetRequest(params: params));
  }
  ...
  Future<int> getTotalUnreadMessageCountWithParams(GroupChannelTotalUnreadMessageCountParams params) async {
    return _int.api.send<int>(UserTotalUnreadMessageCountGetRequest(params: params));
  }
  ...
  int get subscribedCustomTypeTotalUnreadMessageCount {
    return _int.state.unreadCountInfo.customTypes.values.reduce((a, b) => a + b);
  }
  ...
  Stream<Map<String, int>> deliveryStream({String? channelUrl}) async* {
    ...
    await for (final res in _int.streamManager.delivery.stream) {
      if (channelUrl != null) {
        ...
        final status = _int.cache.find<DeliveryStatus>(channelKey: res.channelUrl);
        if (status != null) yield status.updatedDeliveryReceipt;
      }
    }
  }
