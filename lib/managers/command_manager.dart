...
class CommandManager with SdkAccessor {
  ...
  Future<void> _checkDeviceToken(Command cmd) async {
    ...
    if (cmd.payload['application_attributes'].contains('sdk_device_token_cache') == false) {
      ...
      if (cmd.payload.containsKey('device_token_last_deleted_at')) {
        int tokenLastDeletedUnixTime = cmd.payload['device_token_last_deleted_at'] as int;
        ...
        if (deviceTokenUnixTimeAt == null || deviceTokenUnixTimeAt < tokenLastDeletedUnixTime) {
          logger.d(StackTrace.current, error: 'tokenAt NULL');
  ...
  Future<void> _processSessionExpired(Command cmd) async {
    ...
    else if (cmd.payload['expires_in'] != null ? cmd.payload['expires_in'] < 0 : false) {
      await sdk.logout();
    } else {
  ...
  Future<void> _processUpdateMessage(Command cmd) async {
    ...
    try {
      ...
      if (channel is OpenChannel) {
        ...
        if (message.hasUpdatedLaterThan(timestamp) && event.sender?.isCurrentUser == false && !message.isSilent) {
          if (event.hasChangedMentionType() == MentionType.channel && event.previousMentionedContains(currentUser)) {
  ...
  Future<void> _processRead(Command cmd) async {
    try {
      ...
      final hasUnreadCount = (channel.unreadMessageCount > 0 || channel.unreadMentionCount > 0);
      if (isCurrentUser) {
  ...
  Future<void> _processDelivery(Command cmd) async {
    try {
      ...
      if (status.updatedDeliveryReceipt.length == 1 && status.updatedDeliveryReceipt[currUserId] != null) {
        //not to call
  ...
  Future<void> _processChannelPinnedMessage(ChannelEvent event) async {
    try {
      ...
      if (channel is GroupChannel) {
        ...
        } else if ((event.ts ?? 0) > event.data['latest_pinned_message']['updated_at']) {
          channel.pinnedMessageIds = (event.data["pinned_message_ids"]).cast<int>();

          channel.lastPinnedMessage = BaseMessage.fromJson(event.data["latest_pinned_message"]);
          channel.pinnedMessagesUpdatedAt = event.ts ?? 0;
          eventManager.notifyPinUpdated(channel);
        }
      } else {
        final error = SBError(message: "Pin Message Only Supports GroupChannel");
        logger.e(StackTrace.current, error: error);
        throw (error);
      }
    } catch (e) {
  ...
  Future<void> _processBan(ChannelEvent event, bool banned) async {
    try {
      final user = banned ? RestrictedUser.fromJson(event.data) : User.fromJson(event.data);
      final channel = await BaseChannel.getBaseChannel(
        event.channelType,
        event.channelUrl,
      );
  ...
  Future<void> _processMute(ChannelEvent event, bool muted) async {
    try {
      final user = muted ? RestrictedUser.fromJson(event.data) : User.fromJson(event.data);
      final channel = await BaseChannel.getBaseChannel(
        event.channelType,
        event.channelUrl,
      );
      if (channel is GroupChannel) {
        ...
        final member = channel.members.firstWhereOrNull((e) => e.userId == user.userId);
        member?.restrictionInfo = muted ? RestrictionInfo.fromJson(event.data) : null;
  ...
  Future<void> _processChannelMetaData(ChannelEvent event) async {
    try {
      ...
      final cachedMetaData = sdk.cache.find<CachedDataMap<String>>(channelKey: event.channelUrl) ??
          CachedDataMap(
    ...
    try {
      ...
      if (channel is OpenChannel) {
        ...
        channel.members.forEach((member) {
          final isOperator = event.operators.where((e) => e.userId == member.userId).isNotEmpty;
